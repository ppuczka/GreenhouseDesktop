...
namespace GreenhouseDesktopApp
{
    public partial class MainWindow : Window
    {
        ...
        private void ControllerService_OnAlertReceived(object? sender, ControllerAlert alert)
        {
            Dispatcher.Invoke(() =>
            {
                AlertsList.Items.Add($"[{alert.Timestamp:HH:mm:ss}] {alert.Type}: {alert.Message}");
                AlertsList.ScrollIntoView(AlertsList.Items[AlertsList.Items.Count - 1]);
            });
        }
